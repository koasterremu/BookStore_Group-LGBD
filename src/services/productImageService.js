...
export const getProductImages = async (productId) => {
  try {
    const response = await axios.get(
      `${API_URL}/product_image/${productId}`,
      {
        headers: {
...
export const updateProductImage = async (productId, imageId, imageFile) => {
  try {
    ...
    const response = await axios.put(
      `${API_URL}/product_image/${productId}/images/${imageId}`,
      formData,
      {
        headers: {
          Authorization: `Bearer ${getToken()}`, // Thêm token vào header
...
export const deleteProductImages = async (imageId) => {
  try {
    const response = await axios.delete(
      `${API_URL}/product_image/${imageId}`, // Backend API URL for DELETE
      {
        headers: {
...
export const changeIsDefault = async (imageId, isDefault) => {
  try {
    const response = await axios.put(
      `${API_URL}/product_image/image/${imageId}/default`, // Correct URL with imageId
